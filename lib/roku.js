...
this file, via any medium, is strictly prohibited. See LICENSE for full terms.
*/
...
Roku.prototype.launch = function (options, fn) {
  if (typeof options !== 'object') {
    throw new Error('App ID or name are required');
  }

  const appId = options.id;
  const appName = options.name;
  const url = this.url;
  const apps = this.apps.bind(this);

  this.commandQueue.push(function (cb) {
    apps(appId ? { id: appId } : { name: appName }, function (err, app) {
      if (err) {
        if (fn) {
          return fn({
            error: err.desc
      ...
      request.post(url + 'launch/' + Number(app.id), cb);
    });
  });
...
Roku.prototype.apps = function (options, cb) {
  ...
  const allAppsUrl = this.url + 'query/apps';
  const activeAppUrl = this.url + 'query/active-app';
  ...
  if (active) {
    request.get(activeAppUrl, function (err, res) {
    ...
    request.get(allAppsUrl, function (err, res) {
      if (err) {
        return cb(err);
      }
...
Roku.prototype.iconStream = function(appId) {
  return request.get(this.url + 'query/icon/' + appId);
};

Roku.prototype.tvChannels = function (options, cb) {
  if (typeof options === 'function') {
    cb = options;
  }

  const allTvChannelsUrl = this.url + 'query/tv-channels';
  const activeTvChannelUrl = this.url + 'query/tv-active-channel';

  const active = options.active || false;

  if (active) {
    request.get(activeTvChannelUrl, function (err, res) {
      parseString(res.body, function (err, result) {
    ...
    request.get(allTvChannelsUrl, function (err, res) {
      parseString(res.body, function (err, result) {
